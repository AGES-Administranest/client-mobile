...
- **Services that call the API take the id token as a parameter** and pass it to `apiClient` as `{ token }` — the hook gets it from `useAuth().session.idToken`. Never send `userId`: the backend takes the owner from the token and rejects `userId` in bodies/queries with 400. See `features/materials/services/itemService.ts` and the README's *Calling the API*. With no backend yet, stub realistically (`Promise.resolve(...)` with a comment marking it as a stand-in) using the same signature the real call will have.
